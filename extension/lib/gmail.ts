...
export function getRowSubjectElement(row: HTMLTableRowElement): HTMLElement | null {
  const selectors = ['span.bog', 'span.bqe'];
  for (const sel of selectors) {
    const el = row.querySelector<HTMLElement>(sel);
    if (el) return el;
  }
...
export function getRowSenderCell(row: HTMLTableRowElement): HTMLElement | null {
  const selectors = ['td.yX', 'td.yW'];
  for (const sel of selectors) {
    const el = row.querySelector<HTMLElement>(sel);
    if (el) return el;
  }
  const cells = row.querySelectorAll('td');
  if (cells.length > 2) return cells[2] as HTMLElement;
  return null;
}
